...
export default function EditPurchaseOrderPage() {
  ...
  useEffect(() => {
    fetch(`/api/purchase-orders/${poId}`)
      .then(r => r.json())
      .then((j: { purchaseOrder?: { status: string; entityId: string; title: string; description: string | null; type: string; currency: string; spendCategory: string | null; department: string | null; costCentre: string | null; glCode: string | null; validFrom: string | null; validTo: string | null; requiresGoodsReceipt: boolean; requiresContract: boolean; notes: string | null; lineItems: { description: string; quantity: number; unitPrice: number; taxRate: number; glCode: string | null; costCentre: string | null }[] } }) => {
        const po = j.purchaseOrder
        if (!po || po.status !== 'DRAFT') {
          router.push(`/dashboard/purchase-orders/${poId}`)
          return
        }
        setEntityId(po.entityId)
        setTitle(po.title)
        setDescription(po.description ?? '')
        setType(po.type as 'FIXED' | 'OPEN' | 'BLANKET')
        setCurrency(po.currency)
        setSpendCat(po.spendCategory ?? '')
        setDepartment(po.department ?? '')
        setCostCentre(po.costCentre ?? '')
        setGlCode(po.glCode ?? '')
        setValidFrom(po.validFrom ? po.validFrom.slice(0, 10) : '')
        setValidTo(po.validTo ? po.validTo.slice(0, 10) : '')
        setReqGR(po.requiresGoodsReceipt)
        setReqContract(po.requiresContract)
        setNotes(po.notes ?? '')
      ...
      .catch(() => router.push(`/dashboard/purchase-orders/${poId}`))
  ...
  function buildPayload() {
    return {
      title, description: description || null, type, currency,
      spendCategory: spendCat || null, department: department || null,
      costCentre: costCentre || null, glCode: glCode || null,
  ...
  return (
    <div className="p-8 max-w-4xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => router.push(`/dashboard/purchase-orders/${poId}`)}
          className="text-sm" style={{ color: 'var(--muted)' }}>← Back to PO</button>
        <span style={{ color: 'var(--muted)' }}>/</span>
        <span className="text-sm font-medium" style={{ color: 'var(--ink)' }}>Edit Draft</span>
      </div>

      <h1 className="text-2xl font-semibold mb-8" style={{ color: 'var(--ink)' }}>Edit Purchase Order</h1>
      ...
      <section className="mb-8">
        <h2 className="text-sm font-semibold mb-4 uppercase tracking-wide" style={{ color: 'var(--muted)' }}>Header</h2>
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            ...
            <select value={entityId} disabled
              className="w-full text-sm px-3 py-2 rounded-lg border opacity-60 cursor-not-allowed"
              style={{ borderColor: 'var(--border)', background: 'var(--bg)', color: 'var(--ink)' }}>
              {entities.map(e => <option key={e.id} value={e.id}>{e.name}</option>)}
              {!entities.find(e => e.id === entityId) && entityId && (
      ...
      <section className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold uppercase tracking-wide" style={{ color: 'var(--muted)' }}>Line Items</h2>
          <button onClick={addLine} className="text-xs px-3 py-1.5 rounded-lg border font-medium"
            style={{ borderColor: 'var(--border)', color: 'var(--ink)' }}>+ Add Line</button>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)' }}>
                {['Description', 'Qty', 'Unit Price', 'Tax %', 'GL Code', 'Total', ''].map(h => (
                  <th key={h} className="px-3 py-2.5 text-left text-xs font-medium" style={{ color: 'var(--muted)' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={i} style={{ borderTop: i > 0 ? '1px solid var(--border)' : undefined }}>
                  <td className="px-3 py-2">
                    <input value={item.description} onChange={e => updateLine(i, 'description', e.target.value)}
                      placeholder="Description" className="w-full text-sm bg-transparent outline-none"
                      style={{ color: 'var(--ink)' }} />
                  </td>
                  ...
                  <td className="px-3 py-2 w-24">
                    <input value={item.glCode} onChange={e => updateLine(i, 'glCode', e.target.value)}
                      placeholder="GL" className="w-full text-sm bg-transparent outline-none"
                      style={{ color: 'var(--ink)' }} />
                  </td>
                  ...
                  <td className="px-3 py-2 w-8">
                    <button onClick={() => removeLine(i)} className="text-xs opacity-40 hover:opacity-100"
            ...
            <tfoot>
              <tr style={{ borderTop: '2px solid var(--border)', background: 'var(--surface)' }}>
                <td colSpan={5} className="px-3 py-2.5 text-xs font-medium text-right" style={{ color: 'var(--muted)' }}>
                  Grand Total
                </td>
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={saveAndSubmit} disabled={saving || submitting || !title}
