// src/app/api/invoices/[id]/workflow/route.ts
// GET — return the current workflow state for an invoice
...
export async function GET(
  ...
  try {
    ...
    // Find the most recent non-cancelled WorkflowInstance for this invoice
    ...
    // Collect completed step history across all instances for this invoice
    const allInstances = await prisma.workflowInstance.findMany({
      where: { targetObjectType: 'INVOICE', targetObjectId: invoiceId, orgId: session.orgId },
      include: {
        stepInstances: {
    ...
    return handleApiError(err, 'GET /api/invoices/[id]/workflow')
  }
}
